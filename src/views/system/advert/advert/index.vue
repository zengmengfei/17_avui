  ...
  <div class="ele-body">
    <a-card :bordered="false">
      ...
      <ele-pro-table
        ...
        <template #toolbar>
          <a-space>
            <a-button type="primary" @click="openEdit()" v-if="permission.includes('sys:advert:add')">
              <template #icon>
                <plus-outlined/>
              </template>
              <span>新建</span>
            </a-button>
            <a-button type="danger" @click="removeBatch" v-if="permission.includes('sys:advert:dall')">
              <template #icon>
                <delete-outlined/>
              </template>
              <span>删除</span>
        ...
        <template #image_url="{ record }">
          <a-image :width="35" :src="record.image_url"/>
        </template>
        ...
        <template #action="{ record }">
          <a-space>
            <a @click="openEdit(record)" v-if="permission.includes('sys:advert:edit')">修改</a>
            <a-divider type="vertical"/>
            <a-popconfirm
              title="确定要删除此广告吗？"
              @confirm="remove(record)">
              <a class="ele-text-danger" v-if="permission.includes('sys:advert:delete')">删除</a>
            </a-popconfirm>
          </a-space>
        </template>
      </ele-pro-table>
...
import advertEdit from './advert-edit';
...
export default {
  ...
  methods: {
    ...
    remove(row) {
      ...
      this.$http.post('/advert/delete', {id: row.id}).then(res => {
        hide();
        if (res.data.code === 0) {
    ...
    status(checked, row) {
      ...
      this.$http.post('/advert/status', params).then(res => {
        if (res.data.code === 0) {
