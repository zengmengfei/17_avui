  ...
  <div class="ele-body">
    <a-card :bordered="false">
      ...
      <ele-pro-table
        ...
        <template #toolbar>
          <a-space>
            <a-button type="primary" @click="openEdit()" v-if="permission.includes('sys:notice:add')">
              <template #icon>
                <plus-outlined/>
              </template>
              <span>新建</span>
            </a-button>
            <a-button type="danger" @click="removeBatch" v-if="permission.includes('sys:notice:dall')">
              <template #icon>
                <delete-outlined/>
              </template>
              <span>删除</span>
        ...
        <template #source="{ record }">
          <a-tag :color="['green', 'orange'][record.source-1]">
            {{ ['内部通知', '站外新闻'][record.source-1] }}
          </a-tag>
        </template>
        <template #is_top="{ text, record }">
          <a-switch
            :checked="text===1"
            @change="(checked) => editIsTop(checked, record)"/>
        </template>
        ...
        <template #action="{ record }">
          <a-space>
            <a @click="openEdit(record)" v-if="permission.includes('sys:notice:edit')">修改</a>
            <a-divider type="vertical"/>
            <a-popconfirm
              title="确定要删除此职级吗？"
              @confirm="remove(record)">
              <a class="ele-text-danger" v-if="permission.includes('sys:notice:delete')">删除</a>
            </a-popconfirm>
          </a-space>
        </template>
      </ele-pro-table>
